...
@mixin pfe-accordion-variables($state: default, $theme: default, $variant: default) {
  ...
  @if $state != expanded { // Default state
    // Shared variables for all themes
    $variables: (
      BorderColor--accent: transparent, // left border color
      accent: transparent,              // left border color on hover
      BorderColor:         #{pfe-color(surface--border--lighter)},
  ...
  @else { // Expanded state
    ...
    @else { // Defaults for light theme
      $variables: map-merge($variables, (
        ...
        BorderColor--accent: transparent  // left border color when open
      ));
    }
    @if $variant == disclosure {
      $variables: (
...
@mixin pfe-accordion-props($variant: default) {
  ...
  // Border settings (fallback values are used in IE even with other values added before)
  border-top:       #{pfe-local($cssvar: BorderTopWidth, $fallback: #{pfe-var(surface--border-width)}) pfe-var(surface--border-style) pfe-local($cssvar: BorderColor, $fallback: #{pfe-color(surface--border--lighter)})};
  ...
  border-bottom:    #{pfe-local($cssvar: BorderBottomWidth, $fallback: #{pfe-var(surface--border-width)}) pfe-var(surface--border-style) pfe-local($cssvar: BorderColor, $fallback: #{pfe-color(surface--border--lighter)})};
